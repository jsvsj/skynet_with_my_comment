...
struct keyvalue {
	...
	////全局名字的长度  GLOBALNAME_LENGTH  16
	char key[GLOBALNAME_LENGTH];
	
	uint32_t hash;			//保存根据key计算的哈希值
...
//存放keyvalue的指针数组，使用hash实现,使用链地址方法实现的
//数组存储的是一个链表的头节点
struct hashmap {
							//4096
...
//将remote_message_header中的的几个32为整数转换到 message中
//转换为大端字节序
static inline void
